...
9. **기타 업무**(관리자): 채용 외 자유 입력 할 일 관리(`state.tasks`=전역 배열, `{id,title,day,done,createdAt,subs[{id,text,done}]}`). **날짜 타이핑 없이 ‘+ 오늘 / + 내일’ 버튼**으로 등록(`addTask(bucket)`, `day`=오늘/내일 work-date). **업무 기준일은 매일 08:30에 새 ‘오늘’로 전환**(`workToday()`: 08:30 전이면 전날). 미완료 업무는 다음 날 자동 **이월**(`carryOverTasks`: `day<workToday`면 오늘으로 당김, '이월' 배지). 화면=**오늘 할 일 / 내일 할 일 / 완료** 3섹션, 각 카드에 오늘↔내일 이동 버튼·상세 업무(서브태스크) 추가/체크·진행도. 앱 상시 구동 시 `setInterval`로 08:30 경계 통과 감지해 자동 갱신.
