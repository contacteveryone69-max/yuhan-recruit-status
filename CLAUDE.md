...
- **전형 일정표 면접 바 장소 표기·분리**: `calendarHTML` 면접전형은 그룹 키에 **장소(site) 시그니처**를 포함 → 같은 날 같은 기간이라도 **본사/연구소/공장이 다르면 바를 따로 분리**(어느 팀이 어디서 보는지 구분, 레인 누적으로 위아래 배치). 본사 외 장소(연구소/공장)는 `.cb-site` 태그 표기(`g.sites`), 본사는 표기 없음. 지난 면접도 동일하게 site 유지(날짜 필터 없음).
- **면접 일정(예정) 정렬**(`interviewAgendaHTML`): ①면접일 → ②같은 날이면 **장소(본사→연구소→공장, `IV_SITES` 순)** → ③같은 장소면 첫 세션(1차/통합) **시작시간** 순.
...
6. **메일 작성**: 부서 드롭다운 라벨 = `공고시작일(YY.MM.DD) 부서명-채용형태`. 템플릿 = **🔔 부서안내 2종**(공고 문안 확인 요청 / 평가자 계정·서류전형 안내) + **🔔 입사자안내 1종**((경력) 입사 전 제출서류·절차 — 가변 항목만 입력, 네이버폼·검진·서류·교육 안내 고정 양식. 인턴 등 다른 형태는 라벨에 `(형태)` 표기해 추가 예정) + 기존 보고 메일 6종. 모두 사용자 실제 양식 기반. textarea 필드 지원(자격요건·담당업무 등 줄별 → `·`/`1)` 자동 변환). **편의 기능**: ①**메일 앱으로 보내기**(`mailToSend` → `mailto:` 제목·본문 채워 기본 메일 클라이언트 실행, 1800자↑ 잘림 경고) ②**작성자·회신메일 자동 기억**(`MAIL_REMEMBER`+`yh_mail_prefs` localStorage, 필드에 '자동 기억' 표시) ③**미입력 변수 노란색 하이라이트**(`mailHighlight`로 `OOO·O월·O일·O요일·OO시·OO:OO·(직접입력)`를 `.mail-ph`로 강조 — 빠뜨림 방지). *기존 보고 메일은 틀 확정 후 정리 예정.*
7. **통계**: 기간 필터, KPI, 채용형태 분포, 전형 퍼널. **‘최종 채용 인원’ KPI·퍼널 마지막 단계는 `d.completed`(완료된 전형으로 넘어간) 전형의 `counts.hired`만 집계**(`sumDone`) — 진행 중 전형의 채용 수는 미반영. 지원자/서류/AI 합격은 전체 집계. **부서별 상세 표**=컬럼(부서/채용형태/채용요청인원/현재 전형 단계/기준일(공고시작일)/지원자/채용(예정)인원), **헤더 클릭 정렬(오름/내림)**·**팀 검색**·**페이지네이션(기본 15, 30/50/100/전체)**(`renderDeptDetailTable`+`STAT_COLS`, 상태 `statSort/statPage/statPageSize/statSearch`).
- 간편카드 메타에 **헤드헌팅 O/X**(`d.headhunt`) 표시(등록 계정 수 대신). **일반 계정 화면은 완료(`d.completed`) 전형을 숨김**(진행 중만 표시).
8. **일정표**: 월간 캘린더 — **탭 진입 시 항상 오늘 달로 표시**(좌우 화살표로 이동). 면접 일정 상세(`interviewAgendaHTML`)는 **오늘 이후 예정 면접만**, 가까운 면접일이 위로(지난 면접·과거 세션 숨김). 기간 단계는 **연속 간트 바**(주 경계서 분할, 시작·끝 셀에 진한 컬러 border 표시), 동일 단계+동일 기간은 한 바로 병합·마감일 다르면 분리, 레인 누적(자름 없음). 면접전형=빨강(`#C0392B`), 입사=녹색으로 구분. 바 클릭=상세 팝업, 날짜칸(흰 바탕) 클릭=그날 전형 현황 팝업. 헤더 **🖨 면접 일정표 출력**(`openIvPrint`): 면접 세션 등록 부서를 **체크 선택** + **기간(주 단위, 시작주·표시 주 수 1~4·주말 포함 토글)** + 표 제목 → **주간 타임라인 그리드(세로 시간축 1시간 단위 × 요일, 부서 카드는 세션 시작~종료 시각에 비례해 위치·높이 배치, 분 단위도 비례, 같은 날 겹치면 가로 레인 분할; 카드=부서명·채용예정인원·회차별 시간·연구소/공장 태그)** 미리보기 후 `ivPrintNow`로 A4 가로 새 창 인쇄/PDF. 표시 시간 범위는 선택 세션들의 최소 시작~최대 종료(정시 단위)로 자동. 부서별 색상은 `IVP_COLORS` 팔레트를 `ivpDeptList` 순서로 고정 배정.
