...
8. **일정표**: 월간 캘린더 — **탭 진입 시 항상 오늘 달로 표시**(좌우 화살표로 이동). 면접 일정 상세(`interviewAgendaHTML`)는 **오늘 이후 예정 면접만**, 가까운 면접일이 위로(지난 면접·과거 세션 숨김). 기간 단계는 **연속 간트 바**(주 경계서 분할, 시작·끝 셀에 진한 컬러 border 표시), 동일 단계+동일 기간은 한 바로 병합·마감일 다르면 분리, 레인 누적(자름 없음). 면접전형=빨강(`#C0392B`), 입사=녹색으로 구분. 바 클릭=상세 팝업, 날짜칸(흰 바탕) 클릭=그날 전형 현황 팝업. 헤더 **🖨 면접 일정표 출력**(`openIvPrint`): 면접 세션 등록 부서를 **체크 선택** + **기간(주 단위, 시작주·표시 주 수 1~4·주말 포함 토글)** + 표 제목 → **주간 그리드(시간(오전/오후)×요일, 부서별 색상 블록=부서명·채용예정인원·회차별 시간·연구소/공장 태그)** 미리보기 후 `ivPrintNow`로 A4 가로 새 창 인쇄/PDF. 부서별 색상은 `IVP_COLORS` 팔레트를 `ivpDeptList` 순서로 고정 배정.
