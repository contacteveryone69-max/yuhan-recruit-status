...
9. **기타 업무**(관리자): 채용 외 자유 입력 할 일 관리(`state.tasks`=전역 배열, `{id,title,due,done,createdAt,subs[{id,text,done}]}`). 업무 타이핑+일자(선택) 등록 → **일자별 그룹·정렬**(지연→오늘→D-n→기한 미지정), 각 업무에 **상세 업무(서브태스크) 추가/체크**(예: 근로계약서 작성 → EHR 배포 / 작성일자 협의). 완료 토글 시 '완료된 업무' 섹션으로. 요약칩(미완료·지연·오늘 마감·완료)으로 누락 방지. `renderTasks`/`addTask`/`toggleTask`/`addSub` 등.
